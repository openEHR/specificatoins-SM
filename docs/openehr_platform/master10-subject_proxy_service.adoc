...
The Subject Proxy Service (SPS) allows symbolic real world variables characterising the state of a _subject_, which is often a sujbect of care, i.e a patient, but may be some other entity, to be retrieved and potentially tracked over time. Such variables are useful for many kinds of application, since they provide a standard means of obtaining information that concretely may reside in different kinds of back-end systems, without the calling application having to know or care about the particular system, standard or representational model of any variable. Symbolic variables are relied upon by openEHR Task Plans and Decision Logic Modules, within which the conceptual purpose and typical use are described in the {openehr_proc_overview}#_subject_proxy_service[openEHR Process and Planning Overview^].
...
A binding (represented by `ENV_BINDING`) is understood as a set of retrieval methods (e.g. API invocations, queries) each associated with a single subject variable (represented by `VARIABLE_BINDING`), for a particular execution environment. A binding may be loaded in the service via the call `_register_binding()_`. Subject variables at the binding layer are identified by the same identifier used in the proxy layer, i.e. the canonical variable name such as `date_of_birth` acts as the key for each variable binding in the binding layer.

== Persistence

It is assumed that the contents of the SPS are persisted for the life of the system i.e. until the point of any system level re-initialisation actions. The SPS includes a `_reset()_` operation that enables all content to be dumped, returning the service to its virgin state.
